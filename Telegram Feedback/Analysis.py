...
mentor_name = ["Azodov Sarvar", "Olloyorov Sirojiddin", "Rasulov Rahmatulloh", "Shomurodov Sarvarbek", "Shukurov Jasur",
               "Azizova Aziza", "Arslanova Nodira", "Alimbayeva Asalbonu", "Orifjonov Abdulaziz"]
qoniqarsiz = ["Mentor o'z vaqtida ish joyida yo'q", "Mentor umuman yordam bera olmadi",
              "Mentor yordam berishdan bosh tortdi"]
qoniqarli = ["Mentor ish joyiga kech keldi", "Mentor savolimga toliq jovob berolmadi",
             "Mentor javob berdi ammo muomilasizlik blan"]
namunali = ["Mentor o'z vaqtida ish joyida", "Mentor barcha savoimga javob berdi", "Mentor juda ham yaxshi tushuntirdi",
            "Mentor yangicha va qiziqarli usulda taqdimot qilib tushuntirdi"]
...
def qoniqarsiz_hulosa_olish(mentor_data):
    ...
    return pd.DataFrame([[iw_joyi, yordam_berwdan_bow, yordam_berolmadi]],
                        columns=["Mentor o'z vaqtida ish joyida yo'q", "Mentor yordam berishdan bosh tortdi",
                                 "Mentor umuman yordam bera olmadi"])
...
def qoniqarli_hulosa_olish(mentor_data):
    ...
    return pd.DataFrame([[iw_joyi_kech_keldi, toliq_javob_bermadi, muomlasizlik]],
                        columns=[qoniqarli[0], qoniqarli[1], qoniqarli[2]])
...
def all_bar_all(left=None, right=None):
    ...
    ax[1].legend()
    # plt.savefig('image/all_plots.png', bbox_inches='tight', transparent=True)
    # plt.close()
